...
RouteModel::Node *RoutePlanner::NextNode() {
  ...
  auto *first_node = open_list.back();
  open_list.pop_back();
  return first_node;
}
...
RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
  ...
  while (current_node != start_node) {
    ...
    distance += current_node->distance(
        *current_node->parent); // Add distance to parent to toal distance
    current_node = current_node->parent;
    if (current_node == start_node) {
      path_found.push_back(*current_node); // last push back for final node.
    }
  }
